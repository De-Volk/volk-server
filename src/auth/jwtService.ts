...
import { CustomRequest, CustomJwtPayload } from "../types/CustomTypes";
...
const jwtTokenProvider ={
    getUserInfoFromToken: async (req:CustomRequest,res:Response,next:NextFunction) =>{
    ...
    reissueToken: async (req:Request,res:Response) => {
        ...
        try{
            ...
            const decodedAccessToken:CustomJwtPayload = (jwt.verify(accessToken!, process.env.JWT_ACCESS_SECRET!) as JwtPayload);
            const decodedRefreshToken:CustomJwtPayload = (jwt.verify(refreshToken!, process.env.JWT_REFRESH_SECRET!) as JwtPayload);
